...
export const AddProductPage: React.FC<AddProductPageProps> = ({ onSave, onCancel, credits, setCredits }) => {
  ...
  const handleVariantImageClick = (id: string, currentImage: string) => {
      if (currentImage) {
          ...
          updateVariantField(id, 'image', `https://picsum.photos/200/200?random=${randomId}`);
      }
  };
  ...
  const handleSubmit = (e?: React.FormEvent) => {
    ...
    setTimeout(() => {
      const newProduct: Product = {
        ...
        imageUrl: hasVariants && generatedVariants[0]?.image ? generatedVariants[0].image : `https://picsum.photos/400/400?random=${Date.now()}`,
        channels: formData.channels,
        channelPrices: finalChannelPrices, // Store the specific prices
              ...
              return (
                <div className="space-y-6">
                    ...
                    <div className="bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-900/20 dark:to-teal-900/20 p-6 lg:p-8 rounded-2xl border border-emerald-100/50 dark:border-emerald-800/50 shadow-sm relative overflow-hidden">
                        ...
                        <button 
                            className={`w-full mt-6 py-4 rounded-xl font-bold text-lg shadow-lg transition-all active:scale-[0.98] flex items-center justify-center gap-3 relative z-10 ${
                    ...
                    <div className="border-t border-slate-100 dark:border-slate-700 pt-6">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
                            {AVAILABLE_CHANNELS.map(channel => {
                                ...
                                <label 
                                    key={channel.id}
                                    className={`flex items-start gap-3 p-3 rounded-xl border cursor-pointer transition-all duration-300 relative overflow-hidden ${
                                        isSelected
